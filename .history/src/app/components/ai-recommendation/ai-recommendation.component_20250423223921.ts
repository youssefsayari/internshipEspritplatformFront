...
export class AiRecommendationComponent implements OnInit {
  ...
  loadTimeSlots() {
    ...
    this.http.get<any[]>(`http://localhost:8089/innoxpert/api/clients/${this.selectedConsultant.id}/timeslots?date=${this.selectedDate}`)
      .subscribe({
        next: (data) => {
  ...
  bookConsultation() {
    ...
    this.http.post('http://localhost:8089/innoxpert/api/clients/book', payload)
      .subscribe({
        next: () => {
